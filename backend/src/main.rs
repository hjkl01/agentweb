...
async fn main()->Result<()>{
 ...
 db::init(&pool).await?;
 if let Some(password)=db::ensure_default_admin(&pool).await?{eprintln!("\n============================================================");eprintln!(" Agent Web 首次启动，已创建默认管理员账号");eprintln!(" 用户名: admin");eprintln!(" 密码:   {password}");eprintln!(" 请立即保存密码；删除数据库后会重新生成新的密码。");eprintln!("============================================================\n");}
 ...
 let protected_api=Router::new()
  ...
  .route("/agents",get(api::list_agents).post(api::create_agent)).route("/agent-catalog",get(api::catalog)).route("/runtime/settings",get(api::get_runtime_settings).put(api::update_runtime_settings))
  .route("/node/versions",get(api_node::node_versions)).route("/node/install",axum::routing::post(api_node::install_node)).route("/agents/{id}/status",get(api::agent_status)).route("/agents/{id}/models",get(api::agent_models)).route("/agents/{id}/install",axum::routing::post(api::install_agent))
  .route("/sessions",get(api::list_sessions).post(api::create_session)).route("/sessions/{id}",get(api::get_session).delete(api::delete_session)).route("/sessions/{id}/title",axum::routing::put(api::rename_session)).route("/sessions/{id}/pin",axum::routing::put(api::toggle_pin)).route("/sessions/{id}/model",axum::routing::put(api::set_session_model))
