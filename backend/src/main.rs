...
async fn main()->Result<()>{
 ...
 let cleanup_db=pool.clone(); tokio::spawn(async move { let mut ticker=tokio::time::interval(std::time::Duration::from_secs(3600)); loop { ticker.tick().await; let cutoff=Utc::now()-chrono::Duration::days(1); let cutoff_rfc3339=cutoff.to_rfc3339(); let _=sqlx::query("DELETE FROM agent_events WHERE created_at < ? AND (session_id IS NULL OR NOT EXISTS (SELECT 1 FROM sessions WHERE sessions.id=agent_events.session_id AND sessions.status='running'))").bind(&cutoff_rfc3339).execute(&cleanup_db).await; let _=sqlx::query("DELETE FROM login_failures WHERE updated_at < ? AND locked_until <= unixepoch('now')").bind(&cutoff_rfc3339).execute(&cleanup_db).await; } });
 ...
 let app=Router::new().nest("/api",api_routes).merge(SwaggerUi::new("/docs").url("/api-doc/openapi.json",openapi::ApiDoc::openapi())).fallback_service(ServeDir::new(&frontend_dir).not_found_service(ServeFile::new(index_file))).layer(TraceLayer::new_for_http()).with_state(state.clone());
 let bind_addr=std::env::var("AGENTWEB_BIND_ADDR").unwrap_or_else(|_|"0.0.0.0:8080".into());
 let listener=tokio::net::TcpListener::bind(&bind_addr).await?; tracing::info!("Agent Web listening on {bind_addr}");
 let shutdown_db=state.db.clone(); let shutdown_agents=state.agents.clone(); let shutdown_worker=state.worker_id.clone();
 let shutdown=async move {
  #[cfg(unix)] {
   let mut terminate=match tokio::signal::unix::signal(tokio::signal::unix::SignalKind::terminate()){Ok(signal)=>signal,Err(_)=>{let _=tokio::signal::ctrl_c().await; return;}};
   tokio::select!{ _=tokio::signal::ctrl_c()=>{}, _=terminate.recv()=>{} }
  }
  #[cfg(not(unix))] { let _=tokio::signal::ctrl_c().await; }
  tracing::info!("Shutdown signal received, stopping agents owned by worker {}",shutdown_worker);
  let rows=sqlx::query("SELECT id FROM sessions WHERE status='running' AND worker_id=?").bind(&shutdown_worker).fetch_all(&shutdown_db).await.unwrap_or_default();
  let now=Utc::now().to_rfc3339();
  let _=sqlx::query("UPDATE sessions SET status='interrupted',worker_id=NULL,lease_until=NULL,updated_at=? WHERE status='running' AND worker_id=?").bind(&now).bind(&shutdown_worker).execute(&shutdown_db).await;
  for row in rows { let id=sqlx::Row::get::<String,_>(&row,0); let _=shutdown_agents.interrupt(&id).await; }
 };
