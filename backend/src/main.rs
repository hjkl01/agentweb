...
async fn main()->Result<()>{
 ...
 let cleanup_db=pool.clone(); tokio::spawn(async move { let mut ticker=tokio::time::interval(std::time::Duration::from_secs(3600)); loop { ticker.tick().await; let _=sqlx::query("DELETE FROM agent_events WHERE julianday(created_at) < julianday('now','-1 day') AND (session_id IS NULL OR NOT EXISTS (SELECT 1 FROM sessions WHERE sessions.id=agent_events.session_id AND sessions.status='running'))").execute(&cleanup_db).await; let _=sqlx::query("DELETE FROM login_failures WHERE julianday(updated_at) < julianday('now','-1 day') AND locked_until <= unixepoch('now')").execute(&cleanup_db).await; } });
