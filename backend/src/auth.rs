...
pub async fn login(State(state): State<AppState>, headers: HeaderMap, Json(value): Json<LoginRequest>) -> Response {
...
pub async fn change_password(State(state): State<AppState>, headers: HeaderMap, Json(value): Json<ChangePasswordRequest>) -> Response {
    let token = cookie_token_from_headers(&headers);
