...
async fn create_workspace(session_id:&str,requested:Option<&str>)->Result<String,StatusCode>{let base_path=std::env::var("AGENTWEB_WORKSPACE_DIR").unwrap_or_else(|_|"./data/workspaces".into());fs::create_dir_all(&base_path).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;let base=fs::canonicalize(&base_path).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;let name=requested.unwrap_or("").trim();let relative=if name.is_empty()||name=="."{session_id.to_owned()}else{name.to_owned()};let path=FsPath::new(&relative);if path.is_absolute()||path.components().any(|c|matches!(c,Component::ParentDir)){return Err(StatusCode::BAD_REQUEST)}let workspace=base.join(path);fs::create_dir_all(&workspace).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;let canonical=fs::canonicalize(&workspace).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;if !canonical.starts_with(&base){return Err(StatusCode::FORBIDDEN)}Ok(canonical.to_string_lossy().into_owned())}

pub async fn create_session(State(s):State<AppState>,Json(v):Json<CreateSession>)->Result<Json<Session>,StatusCode>{
 let agent_id=match v.agent_id.filter(|v|!v.trim().is_empty()) { Some(id)=>id, None=>crate::api::runtime::setting(&s.db,"default_agent_id").await.filter(|v|!v.trim().is_empty()).ok_or(StatusCode::BAD_REQUEST)? };
 let exists=sqlx::query("SELECT 1 FROM agents WHERE id=? AND installed=1").bind(&agent_id).fetch_optional(&s.db).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?.is_some();
 if !exists{return Err(StatusCode::NOT_FOUND)}
 let model=v.model.or_else(|| futures::executor::block_on(crate::api::runtime::setting(&s.db,"default_model")));
 let id=Uuid::new_v4().to_string();let workspace=create_workspace(&id,v.workspace.as_deref()).await?;let now=Utc::now().to_rfc3339();let title=v.title.filter(|t|!t.trim().is_empty()).unwrap_or_else(||"New Chat".into());
 sqlx::query("INSERT INTO sessions(id,agent_id,title,workspace,status,native_session_id,model,is_pinned,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)").bind(&id).bind(&agent_id).bind(&title).bind(&workspace).bind("idle").bind::<Option<String>>(None).bind(&model).bind(0i64).bind(&now).bind(&now).execute(&s.db).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;
 Ok(Json(Session{id,agent_id,title,workspace,status:"idle".into(),native_session_id:None,model,is_pinned:false}))
}
...
pub async fn set_session_agent(Path(id):Path<String>,State(s):State<AppState>,Json(v):Json<SetAgent>)->Result<Json<Session>,StatusCode>{
 let session=load_session(&s.db,&id).await?;if session.status=="running"{return Err(StatusCode::CONFLICT)}
 let agent_id=v.agent_id.trim();if agent_id.is_empty(){return Err(StatusCode::BAD_REQUEST)}
 let exists=sqlx::query("SELECT 1 FROM agents WHERE id=? AND installed=1").bind(agent_id).fetch_optional(&s.db).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?.is_some();if !exists{return Err(StatusCode::NOT_FOUND)}
 sqlx::query("UPDATE sessions SET agent_id=?,model=NULL,updated_at=? WHERE id=?").bind(agent_id).bind(Utc::now().to_rfc3339()).bind(&id).execute(&s.db).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;Ok(Json(load_session(&s.db,&id).await?))
}
