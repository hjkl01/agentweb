...
async fn create_workspace(session_id:&str,requested:Option<&str>)->Result<String,StatusCode>{
 ...
 fs::create_dir_all(&base_path).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?; let base=fs::canonicalize(&base_path).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;
 let name=requested.unwrap_or("").trim(); let relative=if name.is_empty()||name=="."{session_id.to_owned()}else{name.to_owned()}; let path=FsPath::new(&relative);
 if path.is_absolute()||path.components().any(|c|matches!(c,Component::ParentDir)){return Err(StatusCode::BAD_REQUEST)}
 let workspace=base.join(path); fs::create_dir_all(&workspace).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?; let canonical=fs::canonicalize(&workspace).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;
 if !canonical.starts_with(&base){return Err(StatusCode::FORBIDDEN)} Ok(canonical.to_string_lossy().into_owned())
}

pub async fn create_session(State(s):State<AppState>,Json(v):Json<CreateSession>)->Result<Json<Session>,StatusCode>{
 let exists=sqlx::query("SELECT 1 FROM agents WHERE id=?").bind(&v.agent_id).fetch_optional(&s.db).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?.is_some(); if !exists{return Err(StatusCode::NOT_FOUND)}
 ...
 sqlx::query("INSERT INTO sessions(id,agent_id,title,workspace,status,native_session_id,model,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)").bind(&id).bind(&v.agent_id).bind(&title).bind(&workspace).bind("idle").bind::<Option<String>>(None).bind(&v.model).bind(&now).bind(&now).execute(&s.db).await.map_err(|_|StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn delete_session(Path(id):Path<String>,State(s):State<AppState>)->StatusCode{
 let session=match load_session(&s.db,&id).await{Ok(v)=>v,Err(e)=>return e}; if session.status=="running"{let _=s.agents.interrupt(&id).await;}
 match sqlx::query("DELETE FROM sessions WHERE id=?").bind(&id).execute(&s.db).await{Ok(r) if r.rows_affected()==1=>{let _=fs::remove_dir_all(&session.workspace).await;StatusCode::NO_CONTENT},Ok(_)=>StatusCode::NOT_FOUND,Err(_)=>StatusCode::INTERNAL_SERVER_ERROR}
}
