...
fn inside_workspace(workspace: &Path, path: &Path) -> bool { path.starts_with(workspace) }

pub async fn tree(Query(query): Query<TreeQuery>) -> Result<Json<Vec<TreeItem>>, StatusCode> {
    let workspace = fs::canonicalize(workspace_dir()).await.map_err(|_| StatusCode::NOT_FOUND)?;
    let current = match query.path.as_deref() {
        None | Some("") => workspace.clone(),
        Some(path) => {
            let requested = PathBuf::from(path);
            let absolute = if requested.is_absolute() { requested } else { workspace.join(requested) };
            let canonical = fs::canonicalize(absolute).await.map_err(|_| StatusCode::NOT_FOUND)?;
            if !inside_workspace(&workspace, &canonical) { return Err(StatusCode::FORBIDDEN); }
            canonical
        }
    };
    ...
    while let Some(entry) = entries.next_entry().await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)? {
        ...
        result.push(TreeItem { name, path: path.to_string_lossy().into_owned(), kind: if metadata.is_dir() { "directory" } else { "file" }, size: metadata.len() });
