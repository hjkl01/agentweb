...
async fn detect_agent(db: &sqlx::SqlitePool, id: &str) -> (bool, Option<String>, Option<String>) { let binary = match resolve_agent_binary(db, id).await { Some(v) => v, None => return (false, None, None) }; let version = detect_agent_version(&binary).await; (version.is_some(), version, Some(binary.to_string_lossy().into_owned())) }
...
pub async fn catalog(State(s): State<AppState>) -> Json<Vec<CatalogItem>> {
    let db = &s.db; let c = detect_agent(db, "codex").await; let cl = detect_agent(db, "claude-code").await; let q = detect_agent(db, "qwen-code").await; let g = detect_agent(db, "gemini-cli").await; let o = detect_agent(db, "opencode").await; let p = detect_agent(db, "pi").await; let oc = detect_agent(db, "openclaw").await;
    Json(vec![CatalogItem { id: "codex", name: "Codex", description: "OpenAI coding agent", installed: c.0, requirements: vec!["Node.js"] }, CatalogItem { id: "claude-code", name: "Claude Code", description: "Anthropic coding agent", installed: cl.0, requirements: vec!["Node.js"] }, CatalogItem { id: "qwen-code", name: "Qwen Code", description: "Alibaba Qwen coding agent", installed: q.0, requirements: vec!["Node.js"] }, CatalogItem { id: "gemini-cli", name: "Gemini CLI", description: "Google Gemini coding agent", installed: g.0, requirements: vec!["Node.js"] }, CatalogItem { id: "opencode", name: "OpenCode", description: "Open-source coding agent", installed: o.0, requirements: vec!["Node.js"] }, CatalogItem { id: "pi", name: "Pi", description: "Pi coding agent", installed: p.0, requirements: vec!["Node.js"] }, CatalogItem { id: "openclaw", name: "OpenClaw", description: "General purpose agent", installed: oc.0, requirements: vec!["Node.js"] }])
}
