...
pub(crate) async fn configured_node_path(db: &sqlx::SqlitePool) -> Option<PathBuf> {
...
pub(crate) async fn detect_agent_version(binary: &FsPath) -> Option<String> {
...
async fn detect_agent(db: &sqlx::SqlitePool, id: &str) -> (bool, Option<String>, Option<String>) {
    let binary = match resolve_agent_binary(db, id).await { Some(v) => v, None => return (false, None, None) };
...
pub async fn catalog(State(s): State<AppState>) -> Json<Vec<CatalogItem>> {
    let db = &s.db;
    let c = detect_agent(db, "codex").await;
    let cl = detect_agent(db, "claude-code").await;
    let o = detect_agent(db, "opencode").await;
    ...
    Json(vec![
        ...
        CatalogItem { id: "openclaw", name: "OpenClaw", description: "General purpose agent", installed: oc.0, requirements: vec![] },
    ])
}
