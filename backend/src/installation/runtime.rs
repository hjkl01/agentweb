...
pub fn supported_node_versions() -> Vec<&'static str> {
    vec![
        "26.3.0", "26.2.0", "26.1.0", "24.17.0", "24.16.0", "24.15.0", "22.23.0", "22.22.0",
        "22.21.1",
    ]
}
...
pub async fn available_node_versions() -> Result<Vec<String>> {
    ...
    for url in NODE_INDEX_URLS {
        match client.get(*url).send().await {
            Ok(response) if response.status().is_success() => {
                match response.json::<Vec<NodeRelease>>().await {
                    Ok(items) => {
                        releases = items;
                        break;
                    }
    ...
    let mut by_major: std::collections::BTreeMap<u64, Vec<String>> =
    ...
    for (_, mut releases) in by_major.into_iter().rev() {
        releases.sort_by_key(|a| std::cmp::Reverse(version_key(a)));
        releases.dedup();
...
pub async fn installed_versions() -> Result<Vec<String>> {
    ...
    out.sort_by_key(|a| std::cmp::Reverse(version_key(a)));
    Ok(out)
}
...
pub async fn detect_system_nodes() -> Result<Vec<DetectedNode>> {
    ...
    for path in candidates {
        ...
        if out
            .iter()
            .any(|item: &DetectedNode| item.path == canonical.to_string_lossy())
        {
            continue;
        }
    ...
    out.sort_by_key(|a| std::cmp::Reverse(version_key(&a.version)));
    Ok(out)
}
