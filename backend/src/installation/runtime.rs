...
pub fn node_home(version: &str) -> PathBuf { Path::new(NODE_ROOT).join(format!("v{version}")) }
pub fn node_bin(version: &str) -> PathBuf { node_home(version).join("bin") }

pub async fn installed_versions() -> Result<Vec<String>> {
    let mut out = Vec::new();
    let mut rd = match fs::read_dir(NODE_ROOT).await { Ok(rd) => rd, Err(_) => return Ok(out) };
    while let Some(entry) = rd.next_entry().await? {
        if entry.file_type().await?.is_dir() {
            let name = entry.file_name().to_string_lossy().trim_start_matches('v').to_owned();
            if validate_node_version(&name) && entry.path().join("bin/node").exists() { out.push(name); }
        }
    }
    out.sort(); out.reverse(); Ok(out)
}

pub async fn install_node(version: &str, events: impl Fn(String) + Send + 'static) -> Result<()> {
    if !validate_node_version(version) { return Err(anyhow!("unsupported Node.js version: {version}")); }
    ...
    if !status.success() { return Err(anyhow!("failed to download Node.js {version}")); }
...
pub async fn node_command(version: &str, command: &str) -> Result<Command> {
    if !validate_node_version(version) || !node_bin(version).join("node").exists() { return Err(anyhow!("Node.js {version} is not installed")); }
