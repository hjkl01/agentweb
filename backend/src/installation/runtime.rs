...
/// Backward-compatible fallback for the legacy API endpoint.
pub fn supported_node_versions() -> Vec<&'static str> {
    vec!["26.3.0", "26.2.0", "26.1.0", "24.17.0", "24.16.0", "24.15.0", "22.23.0", "22.22.0", "22.21.1"]
}
...
/// with `false`; older EOL lines are excluded from the selectable list.
pub async fn available_node_versions() -> Result<Vec<String>> {
    ...
    for release in releases {
        ...
        let is_lts = release.lts.as_str().is_some();
        let is_current = release.lts == serde_json::Value::Bool(false) && major >= 26;
...
pub fn node_home(version: &str) -> PathBuf { runtime_root().join(format!("v{version}")) }
pub fn node_bin(version: &str) -> PathBuf { node_home(version).join("bin") }

pub async fn installed_versions() -> Result<Vec<String>> {
    let root = runtime_root();
    let mut out = Vec::new();
    let mut rd = match fs::read_dir(&root).await { Ok(rd) => rd, Err(_) => return Ok(out) };
    while let Some(entry) = rd.next_entry().await? {
        if entry.file_type().await?.is_dir() {
            let name = entry.file_name().to_string_lossy().trim_start_matches('v').to_owned();
            if entry.path().join("bin/node").is_file() { out.push(name); }
        }
    }
...
pub enum NodeSource { Managed, System }

#[derive(Debug, Clone, Serialize)]
pub struct DetectedNode { pub version: String, pub path: String, pub source: NodeSource }
...
pub async fn detect_system_nodes() -> Result<Vec<DetectedNode>> {
    ...
    if let Ok(path) = std::env::var("PATH") {
        for dir in std::env::split_paths(&path) { candidates.push(dir.join("node")); }
    }
    if let Ok(home) = std::env::var("HOME") {
        let nvm_root = PathBuf::from(home).join(".nvm/versions/node");
        if let Ok(mut rd) = fs::read_dir(nvm_root).await {
            while let Some(entry) = rd.next_entry().await? {
                if entry.file_type().await.map(|t| t.is_dir()).unwrap_or(false) { candidates.push(entry.path().join("bin/node")); }
            }
        }
    }
    ...
    for path in candidates {
        let canonical = match fs::canonicalize(&path).await { Ok(p) => p, Err(_) => continue };
        if out.iter().any(|item: &DetectedNode| item.path == canonical.to_string_lossy()) { continue; }
        if let Some(node) = probe_node(&canonical, NodeSource::System).await { out.push(node); }
    }
...
pub async fn detect_nodes() -> Result<Vec<DetectedNode>> {
    ...
    for version in installed_versions().await? {
        out.push(DetectedNode { version: version.clone(), path: node_bin(&version).join("node").to_string_lossy().into_owned(), source: NodeSource::Managed });
    }
...
pub async fn install_node(version: &str, events: impl Fn(String) + Send + 'static) -> Result<()> {
    if !validate_node_version(version).await { return Err(anyhow!("unsupported Node.js version: {version}")); }
    let arch = platform_arch()?;
    let root = runtime_root();
    let home = node_home(version);
    if home.join("bin/node").exists() { return Ok(()); }
    ...
    for url in urls {
        let status = Command::new("curl").args(["-fL", "--retry", "3", "-o"]).arg(&archive).arg(&url).status().await?;
        if status.success() { downloaded = true; break; }
    }
    if !downloaded { return Err(anyhow!("failed to download Node.js {version}")); }
    ...
    let status = Command::new("tar").args(["-xJf"]).arg(&archive).arg("--strip-components=1").arg("-C").arg(&extract_dir).status().await?;
    if !status.success() { return Err(anyhow!("failed to extract Node.js {version}")); }
...
pub async fn node_command(version: &str, command: &str) -> Result<Command> {
    if !node_bin(version).join("node").exists() { return Err(anyhow!("Node.js {version} is not installed")); }
...
pub async fn detect_installed_node() -> Result<Option<(String, String)>> {
    for version in installed_versions().await? {
        ...
        if out.status.success() { return Ok(Some((version, String::from_utf8_lossy(&out.stdout).trim().to_owned()))); }
    }
    Ok(None)
}
