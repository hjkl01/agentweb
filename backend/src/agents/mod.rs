...
impl AgentManager {
 ...
 pub async fn adapter(&self,kind:&str)->Arc<dyn AgentAdapter>{
  ...
  let adapter:Arc<dyn AgentAdapter>=match kind{"codex"=>Arc::new(CodexAdapter::new()),"opencode"=>Arc::new(OpenCodeAdapter::new()),"pi"=>Arc::new(PiAdapter::new()),_=>Arc::new(GenericAdapter::new())};
