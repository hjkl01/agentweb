use crate::{agents::{models::AgentModel, AgentConfig}, events::AgentEvent, installation::runtime, state::AppState};
use axum::{
...
async fn node_bin_dir(db: &sqlx::SqlitePool) -> Option<PathBuf> {
    if let Some(p) = configured_node_path(db).await {
        if p.is_file() { return p.parent().map(PathBuf::from); }
        if p.join("node").is_file() { return Some(p); }
    }
    runtime::detect_installed_node().await.ok().flatten().map(|(v, _)| runtime::node_bin(&v))
}

fn agent_executable(id: &str) -> &str {
...
async fn resolve_agent_binary(db: &sqlx::SqlitePool, id: &str) -> Option<PathBuf> {
    ...
    if out.status.success() { Some(PathBuf::from(String::from_utf8_lossy(&out.stdout).trim())) } else { None }
}

async fn detect_agent_version(binary: &FsPath) -> Option<String> {
    let out = Command::new(binary).arg("--version").output().await.ok()?;
    if !out.status.success() { return None; }
    let version = String::from_utf8_lossy(&out.stdout).trim().trim_start_matches('v').to_owned();
    (!version.is_empty()).then_some(version)
}

async fn detect_agent(db: &sqlx::SqlitePool, id: &str) -> (bool, Option<String>, Option<String>) {
    let binary = match resolve_agent_binary(db, id).await { Some(b) => b, None => return (false, None, None) };
    let version = detect_agent_version(&binary).await;
...
pub async fn catalog(State(s): State<AppState>) -> Json<Vec<CatalogItem>> {
    let db = &s.db;
    let codex = detect_agent(db, "codex").await;
    let claude = detect_agent(db, "claude-code").await;
    let opencode = detect_agent(db, "opencode").await;
    let pi = detect_agent(db, "pi").await;
    let openclaw = detect_agent(db, "openclaw").await;
    Json(vec![
        CatalogItem { id: "codex", name: "Codex", description: "OpenAI coding agent", installed: codex.0, requirements: vec![] },
        CatalogItem { id: "claude-code", name: "Claude Code", description: "Anthropic coding agent", installed: claude.0, requirements: vec!["Node.js"] },
        CatalogItem { id: "opencode", name: "OpenCode", description: "Open-source coding agent", installed: opencode.0, requirements: vec!["Node.js"] },
        CatalogItem { id: "pi", name: "Pi", description: "Pi coding agent", installed: pi.0, requirements: vec!["Node.js"] },
        CatalogItem { id: "openclaw", name: "OpenClaw", description: "General purpose agent", installed: openclaw.0, requirements: vec![] },
    ])
}

#[derive(Serialize)]
pub struct RuntimeSettings { pub node_path: Option<String>, pub agent_paths: HashMap<String, String> }
...
pub async fn get_runtime_settings(State(s): State<AppState>) -> Json<RuntimeSettings> {
    ...
    Json(RuntimeSettings { node_path: setting(&s.db, "node_path").await, agent_paths: paths })
}

#[derive(Deserialize)]
pub struct UpdateRuntimeSettings { pub node_path: Option<String>, pub agent_paths: HashMap<String, String> }
...
#[derive(Serialize)]
pub struct Agent { pub id: String, pub name: String, pub kind: String, pub command: String, pub working_directory: Option<String>, pub installed: bool, pub version: Option<String> }
...
#[derive(Serialize)]
pub struct AgentStatus { pub id: String, pub installed: bool, pub version: Option<String>, pub path: Option<String>, pub runtime: Option<String> }

pub async fn agent_status(Path(id): Path<String>, State(s): State<AppState>) -> Json<AgentStatus> {
    let (installed, version, path) = detect_agent(&s.db, &id).await;
    let runtime = if installed { agent_runtime_label(&id, &s.db).await } else { None };
    Json(AgentStatus { id, installed, version, path, runtime })
}

pub async fn agent_models(Path(id): Path<String>, State(s): State<AppState>) -> Result<Json<Vec<AgentModel>>, StatusCode> {
    let row = sqlx::query("SELECT kind,command,working_directory FROM agents WHERE id=?").bind(&id).fetch_optional(&s.db).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?.ok_or(StatusCode::NOT_FOUND)?;
    let kind: String = row.get(0); let configured_command: String = row.get(1); let working_directory: Option<String> = row.get(2);
    let binary = resolve_agent_binary(&s.db, &id).await.ok_or(StatusCode::NOT_FOUND)?;
    let command = configured_agent_path(&s.db, &id).await.unwrap_or_else(|| binary.to_string_lossy().into_owned());
    let runtime_path = node_bin_dir(&s.db).await.map(|p| p.to_string_lossy().into_owned());
    let cfg = AgentConfig { id: kind.clone(), command: if configured_command.trim().is_empty() { command } else { configured_command }, working_directory, native_session_id: None, runtime_path, model: None };
    let models = s.agents.adapter(&kind).await.list_models(&cfg).await.map_err(|_| StatusCode::BAD_GATEWAY)?;
    Ok(Json(models))
}
...
async fn install_agent_inner(db: &sqlx::SqlitePool, id: &str, events: &crate::events::EventBus) -> bool {
    ...
    if let Some(node_path) = configured_node_path(db).await { if node_path.is_file() { if let Some(home) = node_path.parent().and_then(|x| x.parent()) { cmd.env("NPM_CONFIG_PREFIX", home); } } } else if let Ok(Some((version, _))) = runtime::detect_installed_node().await { cmd.env("NPM_CONFIG_PREFIX", runtime::node_home(&version)); }
    ...
    let binary = node_bin.join(agent_executable(id));
    if !binary.exists() { let now = Utc::now().to_rfc3339(); let _ = sqlx::query("UPDATE agents SET installed=0, version=NULL, updated_at=? WHERE id=?").bind(&now).bind(id).execute(db).await; return false; }
    let version = match detect_agent_version(&binary).await { Some(v) => v, None => { return false; } };
    let now = Utc::now().to_rfc3339(); let _ = sqlx::query("UPDATE agents SET installed=1, version=?, updated_at=? WHERE id=?").bind(&version).bind(&now).bind(id).execute(db).await;
...
pub async fn list_sessions(State(s): State<AppState>) -> Json<Vec<Session>> {
    let rows = sqlx::query("SELECT id,agent_id,title,workspace,status,native_session_id,model FROM sessions ORDER BY updated_at DESC").fetch_all(&s.db).await.unwrap_or_default();
