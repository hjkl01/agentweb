...
pub async fn catalog(State(_state): State<AppState>) -> Json<Vec<CatalogItem>> {
...
#[derive(Serialize)] pub struct MessageItem { pub id:String,pub role:String,pub content:String,pub created_at:String }
pub async fn list_messages(Path(id):Path<String>,State(s):State<AppState>)->Json<Vec<MessageItem>>{let rows=sqlx::query("SELECT id,role,content,created_at FROM messages WHERE session_id=? ORDER BY created_at ASC").bind(&id).fetch_all(&s.db).await.unwrap_or_default();Json(rows.into_iter().map(|r|MessageItem{id:r.get(0),role:r.get(1),content:r.get(2),created_at:r.get(3)}).collect())}
...
pub async fn send_message(Path(id):Path<String>,State(s):State<AppState>,Json(v):Json<MessageReq>)->Json<serde_json::Value>{
    let row=sqlx::query("SELECT agent_id,workspace FROM sessions WHERE id=?").bind(&id).fetch_optional(&s.db).await.ok().flatten();
    let Some(r)=row else{return Json(serde_json::json!({"error":"session not found"}))};
    let agent_id:String=r.get(0); let workspace:Option<String>=r.get(1);
    ...
    let cfg=AgentConfig{id:agent_id.clone(),command:if agent_id=="codex"{"codex exec --json".into()}else{sqlx::query("SELECT command FROM agents WHERE id=?").bind(&agent_id).fetch_optional(&s.db).await.ok().flatten().map(|r|r.get(0)).unwrap_or_default()},working_directory:workspace};
    let manager=s.agents.clone(); let events=s.events.clone(); let db=s.db.clone(); let msg=v.message; let session_id=id.clone();
    tokio::spawn(async move{let adapter=manager.adapter(&cfg.id).await;if let Err(e)=adapter.send_message(&cfg,&session_id,&msg,&events).await{let _=sqlx::query("UPDATE sessions SET status='error',updated_at=? WHERE id=?").bind(Utc::now().to_rfc3339()).bind(&session_id).execute(&db).await;events.publish(AgentEvent::Error{session_id:session_id.clone(),message:e.to_string()});}});
...
async fn websocket(mut socket:WebSocket,s:AppState,session_id:String){let mut rx=s.events.subscribe();while let Ok(event)=rx.recv().await{let matches=match &event{AgentEvent::SessionStarted{session_id:s}|AgentEvent::MessageStarted{session_id:s}|AgentEvent::MessageDelta{session_id:s,..}|AgentEvent::MessageCompleted{session_id:s}|AgentEvent::Error{session_id:s,..}|AgentEvent::SessionCompleted{session_id:s}=>s==&session_id,_=>false};if matches{if let Ok(text)=serde_json::to_string(&event){if socket.send(Message::Text(text.into())).await.is_err(){break;}}}}}
