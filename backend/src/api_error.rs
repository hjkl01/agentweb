...
pub async fn normalize(request:Request<Body>,next:Next)->Response{
 ...
 let (mut parts,body)=response.into_parts();let bytes=to_bytes(body,64*1024).await.unwrap_or_default();
 ...
 let body=Json(json!({"error":{"code":code(status),"message":message}})).into_response().into_body();parts.headers.remove(header::CONTENT_LENGTH);parts.headers.remove(header::CONTENT_TYPE);let mut response=Response::from_parts(parts,body);response.headers_mut().insert(header::CONTENT_TYPE,"application/json".parse().unwrap());response
}
