...
export function App({sessionId,navigate}:Props){const c=useAppController(sessionId,navigate);const[configAgent,setConfigAgent]=useState<Agent>();const[terminalOpen,setTerminalOpen]=useState(false);const activityVersion=useMemo(()=>c.session.activity.map(x=>`${x.key}:${x.type}:${x.detail?.length||0}`).join('|'),[c.session.activity]);useAutoScroll(c.chatRef,{contentVersion:`${c.session.messages.length}:${c.session.stream.length}:${activityVersion}:${sessionId||''}`});const openNewChat=()=>{if(c.runtime.defaultAgentAvailable&&c.runtime.defaultAgentId&&c.runtime.defaultModel)c.createChat();else c.setNewChatOpen(true)};return <div className="app" onClick={()=>c.sessionMenu&&c.setSessionMenu(undefined)}>
 ...
 <AgentCatalog open={c.catalogOpen} agents={c.runtime.agents} node={c.runtime.node} nodeVersion={c.runtime.nodeVersion} installingNode={c.installation.installingNode} installingAgent={c.installation.installingAgent} nodeInstallFeedback={c.installation.nodeInstallFeedback} agentInstallFeedback={c.installation.agentInstallFeedback} defaultAgentId={c.runtime.defaultAgentId} defaultModel={c.runtime.defaultModel} onDefaultChange={c.runtime.saveDefaults} onClose={()=>c.setCatalogOpen(false)} onNodeVersionChange={c.runtime.setNodeVersion} onInstallNode={c.installation.installNode} onInstallAgent={c.installation.installAgent} onInstallCustomAgent={c.installation.installCustomAgent} onConfigureAgent={setConfigAgent}/>
