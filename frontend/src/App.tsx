...
export function App({sessionId,navigate}:Props){const c=useAppController(sessionId,navigate);const[configAgent,setConfigAgent]=useState<Agent>();const activityVersion=useMemo(()=>c.session.activity.map(x=>`${x.key}:${x.type}:${x.detail?.length||0}`).join('|'),[c.session.activity]);useAutoScroll(c.chatRef,{contentVersion:`${c.session.messages.length}:${c.session.stream.length}:${activityVersion}:${sessionId||''}`});return <div className="app" onClick={()=>c.sessionMenu&&c.setSessionMenu(undefined)}>
 ...
 <NewChatDialog open={c.newChatOpen} agents={c.runtime.agents} onClose={()=>c.setNewChatOpen(false)} onCreate={c.createChat} defaultAgentId={c.runtime.defaultAgentId} defaultModel={c.runtime.defaultModel} onDefaultChange={c.runtime.saveDefaults} onManageAgents={()=>{c.setNewChatOpen(false);c.setCatalogOpen(true)}}/>
 <AgentCatalog open={c.catalogOpen} agents={c.runtime.agents} node={c.runtime.node} nodeVersion={c.runtime.nodeVersion} installingNode={c.installation.installingNode} installingAgent={c.installation.installingAgent} nodeInstallFeedback={c.installation.nodeInstallFeedback} onClose={()=>c.setCatalogOpen(false)} onNodeVersionChange={c.runtime.setNodeVersion} onInstallNode={c.installation.installNode} onInstallAgent={c.installation.installAgent} onInstallCustomAgent={c.installation.installCustomAgent} onConfigureAgent={setConfigAgent}/>
