...
export function App({sessionId,navigate}:Props){
 const c=useAppController(sessionId,navigate); const [configAgent,setConfigAgent]=useState<Agent>(); const activityVersion=useMemo(()=>c.session.activity.map(x=>`${x.key}:${x.type}:${x.detail?.length||0}`).join('|'),[c.session.activity]);
 ...
 return <div className="app" onClick={()=>c.sessionMenu&&c.setSessionMenu(undefined)}>
  ...
  <AgentConfigDialog open={Boolean(configAgent)} agent={configAgent} onClose={()=>setConfigAgent(undefined)}/>
