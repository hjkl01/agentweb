...
export function App({ sessionId, navigate }: Props) {
  ...
  const createChat = async (agentId: string, model?: string) => {
    try { const created = await session.createSession(agentId, model); navigate(sessionPath(created.id)); setNewChatOpen(false); setCatalogOpen(false); }
  ...
  const deleteSession = async (id: string) => {
    ...
    try { const next = session.sessions.find(item => item.id !== id)?.id; await session.deleteSession(id); setSessionMenu(undefined); if (sessionId === id) next ? navigate(sessionPath(next)) : navigate('/'); }
    catch (error) { alert(error instanceof Error ? error.message : String(error)); }
  };
