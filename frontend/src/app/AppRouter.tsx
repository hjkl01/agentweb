...
export function AppRouter() {
  ...
  useEffect(() => {
    if (auth === 'authenticated' && window.location.pathname !== '/' && !sessionId) navigate('/', true);
  }, [auth, navigate, sessionId]);

  if (auth === 'checking') {
    return <div className="auth-loading"><div className="auth-loading-mark">A</div><span>正在加载 Agent Web…</span></div>;
  }
