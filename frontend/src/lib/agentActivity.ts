...
export function applyAgentEvent(items: ActivityItem[], event: AgentEvent): ActivityItem[] {
  ...
  const key = activityKey(items, event, data);
  const existing = items.findIndex(item => item.key === key && item.type.startsWith(baseType(event.type)));
  ...
  return [...items, { id: crypto.randomUUID(), key, type: event.type, label, detail }].slice(-MAX_ITEMS);
}
...
function activityKey(items: ActivityItem[], event: AgentEvent, data: Record<string, any>) {
  ...
  const explicit = data.activity_id || data.item_id || data.call_id || data.command_id || data.tool_call_id;
  if (explicit) return String(explicit);

  const base = baseType(event.type);
  const label = labelFor(event, data);
  const isContinuation = !event.type.endsWith('.started');
  if (isContinuation) {
    const active = [...items].reverse().find(item =>
      item.type.startsWith(`${base}.`) &&
