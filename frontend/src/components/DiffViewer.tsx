...
type DiffFile = { name: string; rows: DiffRow[]; status: 'added' | 'deleted' | 'modified' | 'renamed' };
...
function parseFiles(text?: string): DiffFile[] {
  ...
  for (const line of text.split('\n')) {
    if (line.startsWith('diff --git ')) {
      current = { name: line.replace(/^diff --git a\//, '').replace(/ b\/.*$/, ''), rows: [], status: 'modified' };
      files.push(current);
      continue;
    }
    if (!current) continue;
    ...
    else if (line.trim()) current.rows.push({ kind: 'context', text: line });
  }
  return files;
}
...
function DiffFileView({ file }: { file: DiffFile }) {
  ...
  const badge = file.status === 'added' ? 'A' : file.status === 'deleted' ? 'D' : file.status === 'renamed' ? 'R' : 'M';
  return <section className="diff-file">
    <button className="diff-file-head" onClick={() => setOpen(value => !value)}>
      {open ? <ChevronDown size={14} /> : <ChevronRight size={14} />}<span className={`diff-status-badge diff-status-${file.status}`}>{badge}</span><strong>{file.name}</strong>
