...
function TreeNode({ item, depth, sessionId, onSelect }: TreeNodeProps) {
  ...
  const toggle = async () => {
    ...
    setLoading(true);
    try {
      setChildren(await api<Item[]>(`/sessions/${sessionId}/filesystem/tree?path=${encodeURIComponent(item.path)}`));
      setExpanded(true);
    } catch {
      setChildren([]);
    } finally {
  ...
  return (
    <div className="mention-tree-node">
      ...
      {expanded && children.map(child => (
        <TreeNode key={child.path} item={child} depth={depth + 1} sessionId={sessionId} onSelect={onSelect} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="mention-picker" onMouseDown={event => event.preventDefault()}>
      ...
      <div className="mention-tree">
        {loading ? (
          <div className="mention-loading"><LoaderCircle size={16} className="spin" />加载当前工作区…</div>
        ) : visible.length ? (
          visible.map(item => <TreeNode key={item.path} item={item} depth={0} sessionId={sessionId} onSelect={onSelect} />)
        ) : (
