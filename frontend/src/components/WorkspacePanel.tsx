...
export function WorkspacePanel({ current, files, fileFilter, selectedFile, selectedLine, diff, tab, onFilterChange, onSelectTab, onRefresh, onOpenFile, onCloseFile }: Props) {
  return <aside className="workspace">
    ...
    <div className="tabs"><button className={tab === 'files' ? 'active' : ''} onClick={() => onSelectTab('files')}><Folder size={14} /> Files</button><button className={tab === 'diff' ? 'active' : ''} onClick={() => onSelectTab('diff')}><GitCompare size={14} /> Diff</button></div>
    ...
    {selectedFile && <div className="preview"><div className="preview-head"><div><strong>{selectedFile.path}</strong><small>File preview{selectedLine ? ` · line ${selectedLine}` : ''}</small></div><button className="icon-button" onClick={onCloseFile}><X size={15} /></button></div><CodePreview file={selectedFile} focusLine={selectedLine} /></div>}
  </aside>;
}
