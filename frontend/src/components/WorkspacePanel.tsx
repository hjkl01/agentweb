import { Download, File, Folder, GitCompare, RefreshCw, Search, X } from 'lucide-react';
  ...
  return (
    <aside className="workspace">
      <div className="workspace-head">
        <div><strong>Workspace</strong>{current && <small>{current.workspace}</small>}</div>
        <button className="icon-button" onClick={onRefresh} title="Refresh"><RefreshCw size={15} /></button>
      </div>
        ...
        <>
          ...
          <div className="file-list">
            {filteredFiles.map(file => (
              <button className="file-item" key={file.path} onClick={() => onOpenFile(file.path)}>
                <File size={14} /><span>{file.path}</span>
              </button>
            ))}
      ...
      {tab === 'diff' && (
        <div className="diff-panel">
          <pre>{typeof diff === 'string' ? diff : diff?.diff || 'No changes'}</pre>
        </div>
      )}
      ...
      {selectedFile && (
        <div className="file-modal">
          <div className="file-modal-head">
            ...
            <button className="icon-button" onClick={onCloseFile}><X size={15} /></button>
          </div>
