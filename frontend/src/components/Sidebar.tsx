import { Bot, Edit3, LogOut, MessageCircle, MoreHorizontal, Pin, PinOff, Plus, Settings, Terminal, Trash2, UserRound } from 'lucide-react';
import type { Agent, Session } from '../types';

type Props={agents:Agent[];sessions:Session[];active?:string;sessionMenu?:string;onNewChat:()=>void;onSelectSession:(id:string)=>void;onSessionMenu:(id:string)=>void;onRenameSession:(id:string)=>void;onTogglePin:(id:string)=>void;onDeleteSession:(id:string)=>void;onOpenCatalog:()=>void;onOpenAccount:()=>void;onLogout:()=>void;onHome:()=>void;onOpenTerminal:()=>void};
export function Sidebar({agents,sessions,active,sessionMenu,onNewChat,onSelectSession,onSessionMenu,onRenameSession,onTogglePin,onDeleteSession,onOpenCatalog,onOpenAccount,onLogout,onHome,onOpenTerminal}:Props){return <aside className="sidebar">
  ...
  <button className="settings-button terminal-nav-button" onClick={onOpenTerminal}><Terminal size={16}/> Terminal</button>
