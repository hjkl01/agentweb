  ...
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={event => event.stopPropagation()}>
        <div className="modal-head"><div><h2>Agents & runtimes</h2><p>管理可用 Agent 以及运行时环境。</p></div><button className="icon-button" onClick={onClose}><X size={16} /></button></div>
        ...
        <section className="catalog-section">
          ...
          <div className="node-install">
            <select value={nodeVersion} onChange={event => onNodeVersionChange(event.target.value)}>
              ...
              {nodeGroups.map(([major, versions]) => (
                <optgroup key={major} label={`Node ${major}`}>
                  {versions.map(version => <option key={version} value={version}>{version}</option>)}
                </optgroup>
              ))}
            </select>
            <button className="primary-button" onClick={onInstallNode} disabled={!nodeVersion || installingNode}>
              {installingNode ? 'Installing…' : 'Install'}
            </button>
          </div>
        </section>
            ...
            {agents.map(agent => (
              <div className="agent-card" key={agent.id}>
                <div className="agent-info">
                  <div className="agent-name">{agent.name} {agent.installed && <CheckCircle2 size={14} />}</div>
                  <p>{agent.description || 'No description'}</p>
                  {agent.requirements.length > 0 && <small>Requires: {agent.requirements.join(', ')}</small>}
                </div>
                {agent.installed ? <span className="installed-label">Installed</span> : (
                  <button className="primary-button" onClick={() => onInstallAgent(agent.id)} disabled={installingAgent === agent.id}>
                    <Download size={14} /> {installingAgent === agent.id ? 'Installing…' : 'Install'}
                  </button>
                )}
              </div>
            ))}
