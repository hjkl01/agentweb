import { Bot, CheckCircle2, Download, X } from 'lucide-react';
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="agent-panel" onClick={event => event.stopPropagation()}>
        ...
        <div className="runtime-card">
          ...
          <div className="runtime-group"><label>Version</label><div className="runtime-row"><select value={nodeVersion} onChange={event => onNodeVersionChange(event.target.value)}><option value="">Select version</option>{nodeGroups.map(([major, versions]) => <optgroup key={major} label={`Node ${major}`}>{versions.map(version => <option key={version} value={version}>{version}</option>)}</optgroup>)}</select><button onClick={onInstallNode} disabled={!nodeVersion || installingNode}>{installingNode ? 'Installing…' : 'Install'}</button></div></div>
          {node?.installed?.length ? <span className="installed-note">Installed: {node.installed.join(', ')}</span> : null}
        </div>
        <div className="modal-head"><div><h3>Agents</h3><p>按需安装 Codex、Pi、OpenCode 等 Agent。</p></div></div>
        <div className="agent-list">
          {agents.map(agent => <div className="agent-card" key={agent.id}>
            ...
            {agent.installed ? <span className="badge installed">Installed</span> : <button onClick={() => onInstallAgent(agent.id)} disabled={installingAgent === agent.id}><Download size={14} />{installingAgent === agent.id ? 'Installing…' : 'Install'}</button>}
          </div>)}
        </div>
      </div>
    </div>
