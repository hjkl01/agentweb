...
function buildTree(files: FileItem[]): TreeNode[] {
  ...
  for (const file of files) {
    ...
    parts.forEach((name, index) => {
      ...
      if (!node) {
        node = { name, path: currentPath, kind: isLast && file.kind !== 'directory' ? 'file' : 'directory', size: isLast ? file.size : undefined, status: isLast ? file.status : undefined, children: [] };
        children.push(node);
      } else if (isLast && file.status) node.status = file.status;
      children = node.children;
    });
  }
  const sort = (nodes: TreeNode[]) => { nodes.sort((a, b) => a.kind !== b.kind ? (a.kind === 'directory' ? -1 : 1) : a.name.localeCompare(b.name)); nodes.forEach(node => sort(node.children)); };
  sort(root);
  return root;
}
...
function filterTree(nodes: TreeNode[], filter: string): TreeNode[] {
  ...
  return nodes.flatMap(node => { const children = filterTree(node.children, filter); return node.path.toLowerCase().includes(query) || children.length ? [{ ...node, children }] : []; });
}

function collectDirectories(nodes: TreeNode[], result = new Set<string>()) { nodes.forEach(node => { if (node.kind === 'directory') { result.add(node.path); collectDirectories(node.children, result); } }); return result; }
