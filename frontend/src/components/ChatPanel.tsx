  ...
  return (
    <main className="main-panel">
      ...
      <section className="chat" ref={chatRef}>
        ...
        {activity.length > 0 && <div className="activity-card"><button className="activity-header" onClick={onToggleActivity}>{activityOpen ? <ChevronDown size={15} /> : <ChevronRight size={15} />}<Activity size={14} /><span>Agent activity</span><small>{activity.length} events</small></button>{activityOpen && <div className="activity-list">{activity.map(item => <div className="activity-item" key={item.id}><Circle size={7} /><div><span>{item.label}</span>{item.detail && <pre>{item.detail}</pre>}</div></div>)}</div>}</div>}
      </section>

      <div className="composer-wrap"><div className="composer"><textarea ref={textareaRef} value={input} onChange={event => { onInputChange(event.target.value); onAutoResize(); }} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); onSend(); } }} placeholder={active ? 'Message your agent…' : 'Create a chat to start'} disabled={!active || current?.status === 'running'} rows={1} /><button className="send-button" onClick={onSend} disabled={!active || !input.trim() || current?.status === 'running'}>{current?.status === 'running' ? <Square size={16} /> : <Send size={16} />}</button></div><div className="composer-hint">Enter 发送 · Shift + Enter 换行</div></div>
    </main>
  );
}