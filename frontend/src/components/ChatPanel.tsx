...
export function ChatPanel({ current, currentAgent, active, models, modelLoading, messages, stream, activity, activityOpen, input, textareaRef, chatRef, onInputChange, onModelChange, onSend, onToggleActivity, onStop, onNewChat, onAutoResize }: Props) {
  ...
  return (
    <main className="main-panel">
      ...
      <div className="composer-wrap"><div className="composer"><textarea ref={textareaRef} value={input} onChange={event => { onInputChange(event.target.value); onAutoResize(); }} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); onSend(); } }} placeholder={active ? 'Message your agent…' : 'Create a chat to start'} disabled={!active || running} rows={1} /><button className="send-button" onClick={running ? onStop : onSend} disabled={!active || (!running && !input.trim())}>{running ? <><Square size={16} /> Stop</> : <Send size={16} />}</button></div><div className="composer-hint">Enter 发送 · Shift + Enter 换行</div></div>
    </main>
  );
}
