...
export function ChatPanel({ current, currentAgent, active, models, modelLoading, messages, stream, activity, activityOpen, input, textareaRef, chatRef, onInputChange, onModelChange, onSend, onToggleActivity, onStop, onNewChat, onAutoResize }: Props) {
  ...
  return (
    <main className="main-panel">
      ...
      <div className="composer-wrap"><div className="composer"><textarea ref={textareaRef} value={input} onChange={event => { onInputChange(event.target.value); onAutoResize(); }} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey && !running) { event.preventDefault(); onSend(); } }} placeholder={active ? 'Message your agent…' : 'Create a chat to start'} disabled={!active || running} rows={1} /><button className="send-button" onClick={running ? onStop : onSend} disabled={!active || (!running && !input.trim())} aria-label={running ? 'Stop' : 'Send'} title={running ? 'Stop agent' : 'Send message'} style={running ? { width: 36, height: 36, borderRadius: 10, background: '#fff', color: '#555', borderColor: '#d8d8d8', boxShadow: '0 1px 4px #0000000d' } : undefined}>{running ? <Square size={14} strokeWidth={2.2} /> : <Send size={16} />}</button></div><div className="composer-hint">Enter 发送 · Shift + Enter 换行</div></div>
    </main>
  );
}
