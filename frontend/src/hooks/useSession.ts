...
export function useSession(active?: string) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setMessages(loadedMessages); setFiles(loadedFiles); setError(undefined);
    }).catch(error => {
      const message = error instanceof Error ? error.message : String(error);
      console.error('Failed to load session:', error); setError(message);
    });
    ...
    ws.onmessage = event => {
      ...
      if (ev.type === 'message.completed') {
        setStream(value => { if (value) setMessages(items => [...items, { id: crypto.randomUUID(), role: 'assistant', content: value }]); return ''; });
      }
  ...
  const createSession = useCallback(async (agentId: string) => {
    const session = await api<Session>('/sessions', {
      method: 'POST', body: JSON.stringify({ agent_id: agentId, workspace: '.', title: 'New Chat' }),
    });
  ...
  return { sessions, messages, files, stream, activity, activityOpen, error,
    setMessages, setFiles, setActivityOpen, refreshSessions, createSession, sendMessage, deleteSession, interrupt };
}
