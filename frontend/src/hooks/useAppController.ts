...
export function useAppController(sessionId:string|undefined,navigate:(path:string,replace?:boolean)=>void){
 ...
 return {runtime,installation,session,workspace,current,currentAgent,modelState,startupError,input,setInput,catalogOpen,setCatalogOpen,accountOpen,setAccountOpen,newChatOpen,setNewChatOpen,sessionMenu,setSessionMenu,focusLine,setFocusLine,chatRef,textareaRef,logout,createChat,send,renameSession,deleteSession,stop,changeModel,autoResize,openWorkspaceFile};
}
