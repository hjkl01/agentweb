...
export function useAppController(sessionId:string|undefined,navigate:(path:string,replace?:boolean)=>void){
 ...
 const renameSession=async(id:string)=>{const target=session.sessions.find(item=>item.id===id);const title=window.prompt('重命名会话',target?.title||'');if(title===null)return;try{await session.renameSession(id,title);setSessionMenu(undefined)}catch(e){alert(e instanceof Error?e.message:String(e))}};
 const togglePin=async(id:string)=>{try{await session.togglePin(id);setSessionMenu(undefined)}catch(e){alert(e instanceof Error?e.message:String(e))}};
 const deleteSession=async(id:string)=>{if(!confirm('删除这个会话？工作区文件不会被删除。'))return;try{const next=session.sessions.find(x=>x.id!==id)?.id;await session.deleteSession(id);setSessionMenu(undefined);if(sessionId===id)next?navigate(sessionPath(next)):navigate('/')}catch(e){alert(e instanceof Error?e.message:String(e))}};
 ...
 const changeAgent=async(agentId:string)=>{if(!sessionId||current?.status==='running'||agentId===current?.agent_id)return;try{await session.setAgent(sessionId,agentId)}catch(e){alert(e instanceof Error?e.message:String(e))}};
 ...
 return {runtime,installation,session,workspace,current,currentAgent,modelState,startupError,input,setInput,catalogOpen,setCatalogOpen,accountOpen,setAccountOpen,newChatOpen,setNewChatOpen,sessionMenu,setSessionMenu,focusLine,setFocusLine,chatRef,textareaRef,logout,createChat,send,renameSession,togglePin,deleteSession,stop,changeAgent,changeModel,autoResize,openWorkspaceFile};
}
