...
export function useAgentRuntime(){
 ...
 const saveDefaults=useCallback(async(agentId?:string,model?:string)=>{const settings=await api<RuntimeSettings>('/runtime/settings',{method:'PUT',body:JSON.stringify({node_path:node?.configured_path||null,agent_paths:{},default_agent_id:agentId||null,default_model:model||null})});setDefaultAgentId(settings.default_agent_id);setDefaultModel(settings.default_model)},[node?.configured_path]);
