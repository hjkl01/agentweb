...
export function useAgentRuntime(){
 ...
 const refresh=useCallback(async()=>{setError(undefined);try{const [catalog,nodeInfo,settings]=await Promise.all([api<Agent[]>('/agent-catalog'),api<NodeInfo>('/node/versions'),api<RuntimeSettings>('/runtime/settings')]);
   ...
   setAgents(catalog.map((agent,index)=>{const current=status[index];return current?{...agent,installed:current.installed}:agent}));
   setNode(nodeInfo);setNodeVersion(current=>current||nodeInfo.active||'');setDefaultAgentId(settings.default_agent_id);setDefaultModel(settings.default_model)
